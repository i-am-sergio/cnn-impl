...
public:
    ...
    // Entrena red con un ejemplo
    Tensor train(const Tensor& input, const Tensor& target) {
        if (!optimizer) {
            throw std::runtime_error("El modelo no ha sido compilado. Llama a 'compile()' primero.");
        }

        // Asegura de que las capas Dropout esten en modo de entrenamiento
        for (const auto& layer : layers) {
        ...
        Tensor out = forward(input);    // Propagacion hacia adelante

        // Capas Dropout en modo de inferencia para la siguiente llamada a forward 
        for (const auto& layer : layers) {
            if (auto dropout_layer = dynamic_cast<Dropout*>(layer.get())) {
        ...
        Tensor loss_grad;
        if (error_function == "cross-entropy") {
            loss_grad = cross_entropy_derivative(out, target);
        } else {
            loss_grad = mse_derivative(out, target);
        }

        for (int i = (int)layers.size() - 1; i >= 0; --i) { // Propagacion hacia atras
            // Si es una capa Dropout, usar el modo de entrenamiento  para backward
            if (auto dropout_layer = dynamic_cast<Dropout*>(layers[i].get())) {
                dropout_layer->set_training_mode(true);
            }
            loss_grad = layers[i]->backward(loss_grad); // Llama backward
        }

        for (auto& layer : layers) // Actualiza parametros
    ...
    void fit(const vector<Tensor>& X, const vector<Tensor>& Y, 
        ...
        if (!optimizer) throw std::runtime_error("Modelo no compilado. Llamar a 'compile()' primero.");
        ...
        if (training_logs) log_file.close();
    }
